        ...
        switch (type_name) {
            ...
            case 'all': {
                ...
                while (items.length < 10) {
                    ...
                    if (old_item_length === items.length)
                        break;
                }
                sendResults(message, items);
                break;
...
function playSpotify(message, link, transfer, connection) {
    ...
    if (link) {
        if (transfer) {
            spotifyAPI.transferMyPlayback([spotify_config.DEVICE_ID]).then(function () {
                spotifyAPI.play({
    ...
    // else just start playback
        ...
        spotifyAPI.transferMyPlayback([spotify_config.DEVICE_ID], { play: true }).then(function () {
            play(message, connection);
            message.react('▶️');
        }, function (error) {
            console.error('--- ERROR STARTING SPOTIFY PLAYBACK ---\n', error);
